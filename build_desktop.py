...
def main():
    ...
    # Build main DARKI app
    cmd = [
        str(pyinstaller_exe),
        "--windowed",           # Hides command terminal console window
    ...
    print(f"[*] Execute DARKI build command: {' '.join(cmd)}")
    try:
        subprocess.run(cmd, check=True)
        print("[+] PyInstaller DARKI compilation finished successfully.")
    except subprocess.CalledProcessError as e:
        print(f"[!] DARKI compilation failed: {e}")
        sys.exit(1)
    ...
    if dist_dir.exists():
        ...
        # Clean up PyInstaller build artifacts to prevent clutter
        print("[*] Cleaning up build artifacts (.spec files and build/)...")
        try:
            if (root_dir / "build").exists():
                shutil.rmtree(root_dir / "build")
            if (root_dir / "DARKI.spec").exists():
                os.remove(root_dir / "DARKI.spec")
            print("[+] Build artifacts cleaned successfully.")
        except Exception as e:
            print(f"[!] Warning: Could not clean up all build artifacts: {e}")
