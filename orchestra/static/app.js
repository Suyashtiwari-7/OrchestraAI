...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function appendMessageBubble(role, text, metadata = null, updateLocalState = true) {
        ...
        let optionsHTML = "";
        if (role === "assistant" && metadata && metadata.options && metadata.options.length > 0) {
            optionsHTML = `<div class="interactive-options">`;
            metadata.options.forEach(opt => {
                const safeVal = opt.value.replace(/'/g, "\\'");
                optionsHTML += `<button class="interactive-option-btn" onclick="window.sendOptionText('${safeVal}')">${opt.label}</button>`;
            });
            optionsHTML += `</div>`;
        }
    ...
    // Send option text back to chat
    window.sendOptionText = function(text) {
    ...
    // ============================================
    // JARVIS Telemetry Dashboard — live system metrics
    // ============================================
    async function updateTelemetry() {
        try {
            const [teleRes, windowRes] = await Promise.all([
                fetch("/api/telemetry").then(r => r.json()).catch(() => null),
                fetch("/api/context/active_window").then(r => r.json()).catch(() => null),
            ]);

            if (teleRes && teleRes.success) {
                const t = teleRes.telemetry;

                // CPU
                const cpuBar = document.getElementById("tele-cpu-bar");
                const cpuVal = document.getElementById("tele-cpu");
                if (cpuBar && cpuVal) {
                    cpuBar.style.width = t.cpu_percent + "%";
                    cpuBar.classList.toggle("high", t.cpu_percent > 80);
                    cpuVal.textContent = t.cpu_percent + "%";
                }

                // RAM
                const ramBar = document.getElementById("tele-ram-bar");
                const ramVal = document.getElementById("tele-ram");
                if (ramBar && ramVal) {
                    ramBar.style.width = t.ram_percent + "%";
                    ramBar.classList.toggle("high", t.ram_percent > 85);
                    ramVal.textContent = t.ram_percent + "%";
                }

                // Disk
                const diskBar = document.getElementById("tele-disk-bar");
                const diskVal = document.getElementById("tele-disk");
                if (diskBar && diskVal) {
                    diskBar.style.width = t.disk_percent + "%";
                    diskBar.classList.toggle("high", t.disk_percent > 90);
                    diskVal.textContent = t.disk_percent + "%";
                }

                // Battery
                const batBar = document.getElementById("tele-bat-bar");
                const batVal = document.getElementById("tele-bat");
                if (batBar && batVal && t.battery_percent !== null) {
                    batBar.style.width = t.battery_percent + "%";
                    batBar.classList.toggle("low", t.battery_percent < 20);
                    let batText = t.battery_percent + "%";
                    if (t.battery_plugged) batText += " ⚡";
                    batVal.textContent = batText;
                }
            }

            // Active window
            if (windowRes && windowRes.success) {
                const windowTitle = document.getElementById("tele-window-title");
                if (windowTitle) {
                    let title = windowRes.window_title || "--";
                    if (title.length > 35) title = title.substring(0, 35) + "…";
                    windowTitle.textContent = title;
                    windowTitle.title = windowRes.window_title;
                }
            }
        } catch (e) {
