...
def main():
    ...
    try:
        ...
        if tray_icon:
            tray_icon.show()
            widget.set_tray_icon(tray_icon)
        ...
        # Automatically open the full chat window on startup to provide immediate visual feedback
        # QTimer.singleShot(1000, open_full_chat) # Disabled per user request so only desktop widget shows

        # --- 6. Live Voice Session ---
        voice_session = None
        try:
            from orchestra.voice.live_session import LiveVoiceSession, VoiceState

            def on_voice_command(text):
                """Called from voice thread — cross into Qt thread to submit command."""
                QTimer.singleShot(0, lambda: widget.submit_command(text))
                return ""

            def on_state_change(state):
                """Map voice session state to mascot animation."""
                state_map = {
                    VoiceState.LISTENING: "listening",
                    VoiceState.THINKING: "working",
                    VoiceState.SPEAKING: "working",
                    VoiceState.IDLE: "idle",
                }
                anim = state_map.get(state, "idle")
                QTimer.singleShot(0, lambda: widget.robot.set_state(anim))
            ...
            voice_session.start()
            logger.info("Live voice session started (Faster-Whisper + Kokoro-82M).")
        except Exception as e:
            logger.warning(f"Live voice session failed to start: {e}")
        ...
        try:
            ...
            hotkey_listener = HotkeyListener(on_activate=on_hotkey)
            hotkey_listener.start()
            logger.info("Global hotkeys registered: Ctrl+0 and Ctrl+Numpad0")
        except Exception as e:
        ...
        logger.info("  - Continuous live voice enabled")
