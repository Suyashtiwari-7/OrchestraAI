...
class ProviderName(Enum):
    ...
    GROQ = "groq"
    OLLAMA = "ollama"
    GEMINI = "gemini"       # Kept solely for image generation (Imagen 3)
...
# ============================================
MODELS = {
    # --- NVIDIA NIM (Free tier — primary heavy-lift models) ---
    "nim-llama": ModelConfig(
        provider=ProviderName.NVIDIA_NIM,
        model_id="meta/llama-3.3-70b-instruct",
        display_name="Llama 3.3 70B (NVIDIA NIM)",
        max_tokens=4096,
        temperature=0.7,
    ),
    "nim-deepseek": ModelConfig(
        provider=ProviderName.NVIDIA_NIM,
    ...
    # --- Groq (Ultra-fast utility — 500+ tok/sec) ---
    ...
    # --- Local Ollama (100% offline fallback) ---
    ...
    # --- Google Gemini (Kept solely for image generation) ---
...
ROUTING_TABLE: dict[TaskType, RouteConfig] = {
    ...
    TaskType.AGENTIC_CHAIN: RouteConfig(
        primary="nim-deepseek",
        fallback="groq-deepseek",
        description="Plan and execute multi-step complex workflows",
    ),
}
...
class APIKeys:
    ...
    gemini: Optional[str] = None     # Kept solely for image generation
    ...
    def get_key(self, provider: ProviderName) -> Optional[str]:
        ...
        key_map = {
            ProviderName.NVIDIA_NIM: self.nvidia_nim,
    ...
    def validate(self) -> dict[ProviderName, bool]:
        ...
        return {
            ProviderName.NVIDIA_NIM: bool(self.nvidia_nim and self.nvidia_nim != "your_nvidia_nim_api_key_here"),
...
class AppSettings:
    ...
    # Classifier
    classifier_model: str = "groq-llama"
    ...
    # Voice / TTS Settings
    voice_enabled: bool = field(default_factory=lambda: os.getenv("VOICE_ENABLED", "false").lower() == "true")
    voice_mode: str = field(default_factory=lambda: os.getenv("VOICE_MODE", "offline"))  # "offline" or "online"
    voice_name: Optional[str] = field(default_factory=lambda: os.getenv("VOICE_NAME", "en-US-GuyNeural"))

    # Plugin Settings
    plugins_dir: Path = field(default_factory=lambda: PROJECT_ROOT / "plugins")

    # Context Awareness Settings
    context_awareness: bool = True  # Track active window for system prompt injection
    ...
    def get_imap_accounts(self) -> list:
        ...
        # 1. Retrieve password mappings from env
        password_map = {}

        # Read from IMAP_PASSWORDS JSON dictionary
        ...
        # Read from IMAP_ACCOUNTS JSON list
