...
def execute_system_command(response_content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        from .system_control import set_volume, get_volume, set_brightness, get_brightness, lock_screen, take_screenshot, get_system_info, set_power_mode
        ...
        if action == "open_browser":
            ...
            return {
                ...
                "command": target,
                "reasoning": reasoning,
                "details": f"Executing terminal command: '{target}'"
            }
            ...
            return {
                ...
                "command": target,
                "reasoning": reasoning,
                "details": f"Executing Python script in sandbox."
            }
        ...
        elif action == "system_power_mode":
            res = set_power_mode(target)
            if res.get("success"):
                return {
                    "success": True,
...
def install_application(app_name: str) -> Dict[str, Any]:
    """
    Attempts to install an application using dedicated cross-platform AppInstaller (winget/brew).
    """
    from .app_installer import install_app
